...
export default function Stream(props) {
  ...
  // Localstroage key
  const LOCAL_STORAGE_KEY = "AnimeMadara-netlify-app";
  useEffect(() => {
    ...
    <>
      <Helmet>
        ...
        <meta
          ...
          charSet="utf-8"
        />
        <title>{`Watching ${detail.animeTitle} on AnimeMadara`}</title>
      ...
      <div className="footer">
        <div className="footer-content">
          <div className="footer-icons">
            ...
            <i class="fa-brands fa-discord" onClick={() => openInNewTab("https://discord.gg")}></i>
            <i class="fa-brands fa-github" onClick={() => openInNewTab("https://github.com/madhvendra-singh")}></i>
          </div>
          <h3>Anime <span>Madara</span></h3>
          <p>AnimeMadara is not affiliated with or endorsed by any of the anime studios behind the creation of the anime presented on this site. This website is only an user interface presenting/linking various self-hosted files across the internet by other third-party providers for easy access . AnimeMadara never downloads the video from any source provider, link will be returned from the response hence it is completely not subjected to DMCA compliant</p>
        </div>
      </div>
    </>
  );
